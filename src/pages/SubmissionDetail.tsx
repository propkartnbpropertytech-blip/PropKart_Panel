...
• Maps: ${sub.location_url || 'N/A'}`;
  ...
  return (
    <div className="p-6 space-y-6">
      {/* Toast message */}
      {toastMessage && (
        <div className="fixed bottom-6 right-6 z-50 px-4 py-3 rounded-2xl bg-slate-800 border border-slate-700 text-white text-xs font-medium shadow-2xl flex items-center gap-2 animate-in fade-in slide-in-from-bottom-5">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-sm">
          ...
          <div>
            <div className="flex items-center gap-2.5">
              ...
              <button
                ...
                title="Copy ID"
              >
                {copiedKey === 'Reference ID' ? <Check className="w-4 h-4 text-emerald-400" /> : <Copy className="w-4 h-4" />}
              </button>
              <span className="text-xs text-slate-400">• Version {schema?.version?.version_number || 1}</span>
            </div>
            <div className="flex items-center gap-2 text-xs text-slate-400 mt-1">
              <span>Submitted by <strong className="text-white">{submission.owner_name}</strong></span>
              <span>•</span>
          ...
          <button
            ...
            className={`inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-semibold shadow-md transition-all ${
              submission.status === 'Converted'
                ...
                : 'bg-brand-600 hover:bg-brand-500 text-white shadow-brand-950/30'
            }`}
          >
            {converting ? (
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-sm space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-[11px] text-slate-400 mb-1.5">Current Lead Status</label>
                <select
                  value={submission.status}
                  onChange={(e) => handleStatusChange(e.target.value)}
                  className="w-full px-3.5 py-2.5 rounded-xl bg-slate-950 border border-slate-800 text-white text-xs font-semibold focus:outline-none focus:border-brand-500"
                >
              ...
              <div>
                <label className="block text-[11px] text-slate-400 mb-1.5">Assigned Telecaller</label>
                <select
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {sec.fields.map((f) => {
                  ...
                  if (f.field_type === 'google_location') {
                    ...
                    return (
                      <div key={f.field_key} className="md:col-span-2 p-4 rounded-2xl bg-slate-950 border border-slate-800/80 space-y-2">
                  ...
                  if (f.field_type === 'direction') {
                    ...
                    return (
                      <div key={f.field_key} className="md:col-span-2 p-4 rounded-2xl bg-slate-950 border border-slate-800/80 space-y-1">
                        <div className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">
                          {f.label}
                        </div>
                        <div className="flex items-center justify-between gap-3">
                          <span className="text-xs text-white leading-relaxed">{val || 'N/A'}</span>
                          {isLink && (
                            <a
                              href={String(val)}
                              target="_blank"
                              rel="noopener noreferrer"
                              className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg bg-slate-800 text-brand-400 hover:text-brand-300 text-xs font-semibold shrink-0"
                            >
                        ...
                        <div className="flex flex-wrap gap-2">
                          {items.length === 0 ? (
                            ...
                            items.map((item, idx) => (
                              <span
                                key={idx}
                                className="px-2.5 py-1 rounded-lg bg-brand-600/10 border border-brand-500/20 text-brand-300 text-xs font-medium"
                              >
                                {item}
                              </span>
                            ))
                  ...
                  return (
                    <div
                      key={f.field_key}
                      className="p-3.5 rounded-2xl bg-slate-950 border border-slate-800/70 space-y-1"
                    >
        ...
        <div className="space-y-6">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-sm space-y-4">
            ...
            <form onSubmit={handleAddNote} className="space-y-3">
              <div>
                <label className="block text-[11px] text-slate-400 mb-1">Call Status Outcome</label>
                <select
                  value={noteStatus}
              ...
              <div>
                <label className="block text-[11px] text-slate-400 mb-1">Call Notes / Discussion</label>
                <textarea
                  rows={3}
                  value={newNote}
                  onChange={(e) => setNewNote(e.target.value)}
                  placeholder="Record key details: owner availability, price flexibility, documents verified..."
                  className="w-full px-3.5 py-2.5 rounded-xl bg-slate-950 border border-slate-800 text-white text-xs leading-relaxed focus:outline-none focus:border-brand-500"
                />
              </div>
              ...
              <button
                ...
                className="w-full py-2.5 rounded-xl bg-brand-600 hover:bg-brand-500 text-white text-xs font-semibold shadow-sm flex items-center justify-center gap-2 disabled:opacity-40"
              >
            ...
            {/* Notes Timeline */}
            <div className="pt-3 border-t border-slate-800/80 space-y-3 max-h-80 overflow-y-auto">
              {notes.length === 0 ? (
                ...
                notes.map((n) => (
                  ...
                  >
                    ...
                    {n.call_status && (
                      <span className="inline-flex px-2 py-0.5 rounded-md text-[10px] font-bold bg-slate-800 text-amber-300 border border-slate-700">
                        {n.call_status}
                      </span>
                    )}
